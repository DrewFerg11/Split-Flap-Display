...
Each board listed here is a drop-in per-module driver: one board per split-flap module, daisy-chained via NEXT/PREV headers with no wiring between boards.
...
!!! info "More boards coming"
    Additional community-designed PCBs are in the works. When available, they'll be listed here with their own guides.
